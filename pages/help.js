  ...
  return (
    <div className="flex flex-1 flex-col w-full card-sm:px-20 items-center bg-gradient-radial from-slate-500 via-slate-600 to-slate-900 overflow-y-auto">
        ...
        <div className="p-4 m-4 bg-orange-200 border-4 border-black">
            ...
            <p className="card-sm:text-xl font-bold indent-10">Starting at the <a className='text-blue-500' href={`/strategy`}>strategy page</a>, you can see the available events in the upper left hand corner. Clicking on an event will populate the stages in that event. To see the teams posted on the stage, you click on the events and the middle column will populate with the teams posted to that stage. If no teams are posted, a propmt will appear to state that there have been no teams posted for the stage yet. If a team is posted, you may click on the team to show the details of how that user completed the stage with that team.</p>
            {/* ADD VIDEO */}
            <p className="font-header text-3xl pt-4">Posting a Team To a Stage</p>
            <p className="card-sm:text-xl font-bold indent-10">Once a stage has been selected you can see the option 'ADD TEAM TO STAGE'. In order to add a team to a stage, you need to be logged in. Once the 'ADD TEAM TO STAGE' has been clicked, you can see a prompt which gives you two options, you can post a team that you have built at the <a className='text-blue-500' href={`/`}>home page</a> or make a team from scratch.</p>
            {/* ADD VIDEO */}
            <p className="card-sm:text-xl font-bold indent-10">Once an option has been selected, a form should appear. The first thing you should see is the team layout: leader, sub-leader, first rotation, second rotation, and the float characters. If you selected a pre-built team and this is not how you want to orient the team, please got back to your deck at the <a className='text-blue-500' href={`/`}>home page</a> to edit your team accordingly. Below the team layout is the Team Information. This is where you fill out the information for the team used to clear the stage. Starting from the top, you can input a team name. Also, if there are missions on that stage (besides just clearing the stage) these will appear under the team name. If your team accomplished any of the missions, feel free to tag select it. Going down further, you can see each character has three separate inputs: EZA, Hidden Potential, and Character Strategy. If a character is EZA'd, you can select the box. The Hidden Potential orbs can be selected to highlight them, showing that path has been unlocked. Then the character strategy is for any other information you want players to know.</p>
            {/* ADD VIDEO HERE */}
